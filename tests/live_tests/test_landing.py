...
class TestLanding(unittest.TestCase):
    ...
    def test_metric_change(self):
        with inject_timestamps_tests(app, timestamp, website_timestamp), app.test_client() as client:
            response = client.get('/')
            data = response.data
            cases_change, cases_percentage_change = calculate_change(cases_metric)
            deaths_change, deaths_percentage_change = calculate_change(deaths_metric)
            admissions_change, admissions_percentage_change = calculate_change(healthcare_metric)
            tests_change, tests_percentage_change = calculate_change(testing_metric)
    ...
    def test_date_range(self):
        ...
        data = response.data
        cases_latest_date, cases_current_week_date, cases_latest_week_ago_date, cases_date_fortnight_prior = get_date_min_max(cases_metric)
        deaths_latest_date, deaths_current_week_date, deaths_latest_week_ago_date, deaths_date_fortnight_prior = get_date_min_max(deaths_metric)
        admissions_latest_date, admissions_current_week_date, admissions_latest_week_ago_date, admissions_date_fortnight_prior = get_date_min_max(healthcare_metric)
        tests_latest_date, tests_current_week_date, tests_latest_week_ago_date, tests_date_fortnight_prior = get_date_min_max(testing_metric)
